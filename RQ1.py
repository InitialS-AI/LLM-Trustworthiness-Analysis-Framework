...
def rq1(state_abstract_args, prob_args, train_instances, val_instances, test_instances):
    ...
    results["transition_KL_divergence_instance_level"] = metrics_obj.transition_KL_divergence_instance_level()
    results["transition_MMD_instance_level"] = metrics_obj.transition_MMD_instance_level()
...
for dataset, optimal_setting in datasets.items():
    ...
    train_instances_loaded, val_instances_loaded, test_instances_loaded = load_data(state_abstract_args)
    ...
    results = rq1(state_abstract_args, prob_args, train_instances, val_instances, test_instances)

    # transition_gain, normal_coverage_dict, abnormal_coverage_dict = results["transition_gain"]

    transition_distribution_divergence = results["transition_KL_divergence_instance_level"]
    ...
    # For test_probs
    sampled_indices_test = np.random.choice(np.arange(len(test_probs)), size=num_samples, p=test_probs / sum(test_probs))

    sns.kdeplot(sampled_indices_test, fill=True, color='darkred', label="TestAbnormal", linewidth=2, ax=axs_kde[idx])

    sampled_indices_val = np.random.choice(np.arange(len(val_probs)), size=num_samples, p=val_probs / sum(val_probs))
    sns.kdeplot(sampled_indices_val, fill=True, color='darkblue', label="TestNormal", linewidth=2, ax=axs_kde[idx])

    sampled_indices_train = np.random.choice(np.arange(len(train_probs)), size=num_samples, p=train_probs / sum(train_probs))
    sns.kdeplot(sampled_indices_train, fill=True, color='darkgreen', label="TrainNormal", linewidth=2, ax=axs_kde[idx])
    ...
    transition_distribution_divergence = np.array(transition_distribution_divergence)
    transition_distribution_divergence = transition_distribution_divergence[np.isfinite(transition_distribution_divergence)]
    ...
    # Mann-Whitney U-test
    u_statistic, p_value = mannwhitneyu(val_probs, test_probs, alternative='two-sided')
